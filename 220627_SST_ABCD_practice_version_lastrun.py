...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\OSLadmin\\Desktop\\220707SST_psychopy\\220627_SST_ABCD_practice_version_lastrun.py',
...
for thisTrials_loop in trials_loop:
    ...
    while continueRoutine:
        ...
        if key_resp_go_stop_trial.status == STARTED and not waitOnFlip:
            ...
            if len(_key_resp_go_stop_trial_allKeys):
                ...
                # was this correct?
                if (key_resp_go_stop_trial.keys == str(updated_go_stimuli_correct_answer)) or (key_resp_go_stop_trial.keys == updated_go_stimuli_correct_answer):
                    key_resp_go_stop_trial.corr = 1
                else:
                    key_resp_go_stop_trial.corr = 0
                # a response ends the routine
                continueRoutine = False
    ...
    if key_resp_go_stop_trial.keys in ['', [], None]:  # No response was made
        ...
        if str(updated_go_stimuli_correct_answer).lower() == 'none':
           key_resp_go_stop_trial.corr = 1;  # correct non-response
        else:
           key_resp_go_stop_trial.corr = 0;  # failed to respond (incorrectly)
    # store data for trials_loop (TrialHandler)
    trials_loop.addData('key_resp_go_stop_trial.keys',key_resp_go_stop_trial.keys)
    trials_loop.addData('key_resp_go_stop_trial.corr', key_resp_go_stop_trial.corr)
    ...
    if trial_type == 'VariableStopTrial':
        ...
        if STEflag == 0:
            ...
            if key_resp_go_stop_trial.keys == str(updated_go_stimuli_correct_answer):
